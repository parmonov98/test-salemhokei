          ...
          '<div>' +
            '<label class="btn btn-default btn-file" style="margin: 10px 0 0;">' +
              ...
              '<input type="file" id="employees_{{ $lang }}[' + (count + 1) + '][img]"
                name="employees_{{ $lang }}[' + (count + 1) + '][img]" style="display: none;">' +
              '</label>' +
            '</div>' +
          '</div>' +
        '<div class="form-group">' +
...
<div class="right_col" role="main">
    <div class="">
        ...
        <div class="row">
                            ...
                            <div class="" role="tabpanel" data-example-id="togglable-tabs">
                                ...
                                <div id="myTabContent" class="tab-content">
                                    ...
                                    <div role="tabpanel" class="tab-pane fade" id="tab_public_url"
                                        ...
                                        <h2>
                                            <a target="_blank" href="/kk/pages/{{$item->alias}}">{{$item->alias}}</a> -
                                            KK
                                            <br />
                                            <a target="_blank" href="/ru/pages/{{$item->alias}}">{{$item->alias}}</a> -
                                            RU
                                        </h2>
                                    </div>
                                </div>
                        ...
                        <div class="x_content" style="display: none;">
                            <div class="" role="tabpanel" data-example-id="togglable-tabs">
                                ...
                                <div id="myTabContent" class="tab-content">
                                    ...
                                    @foreach ($langs as $key => $lang)
                                    <div role="tabpanel" class="tab-pane fade @if ($key==0) active in @endif"
                                        id="tab_employees_{{ $lang }}_content" aria-labelledby="home-tab">
                                        <div id="employees_{{ $lang }}">
                                            <div class="fields">
                                                @if ($item->getAttribute('employees_' . $lang))
                                                @foreach ($item->getAttribute('employees_' . $lang) as $key =>
                                                $accordion)
                                                <input type="hidden" name="employees_{{ $lang }}[{{ $key }}][old_img]"
                                                    value="{{ $accordion['img'] }}">
                                                <div class="dynamic_field" data-key="{{ $key }}">
                                                    <div class="form-group">
                                                        <label
                                                            for="employees_{{ $lang }}[{{ $key }}][img]">Изображение</label>
                                                        <div>
                                                            ...
                                                            <label class="btn btn-default btn-file"
                                                                ...
                                                                <span>Загрузить изображение</span>
                                                                <input type="file"
                                                                    id="employees_{{ $lang }}[{{ $key }}][img]"
                                                                    name="employees_{{ $lang }}[{{ $key }}][img]"
                                                                    style="display: none;">
                                                            </label>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div class="form-group">
                                                        <label for="employees_{{ $lang }}['title'][]">ФИО</label>
                                                        <input type="text"
                                                            name="employees_{{ $lang }}[{{ $key }}][title]"
                                                            class="form-control" placeholder="Введите ФИО"
                                                            value="{{ $accordion['title'] }}">
                                                    </div>
                                                    <div class="form-group">
                                                        <label
                                                            for="employees_description_{{ $lang }}_{{ $key }}">Описание</label>
                                                        <textarea name="employees_{{ $lang }}[{{ $key }}][description]"
                                                            id="employees_description_{{ $lang }}_{{ $key }}" cols="30"
                                                            rows="3"
                                                            class="form-control">{!! $accordion['description'] !!}</textarea>
                                                    </div>
                                                    <hr>
            ...
            @can('page.delete')
            @if (!empty($item))
            <form action='{{ route('page.delete', ['item', $item->id]) }}' method="POST">
                {{ csrf_field() }}
                {{ method_field('DELETE') }}
                <div class="col-md-12 col-sm-12 col-xs-12">
    ...
    @foreach ($languages as $lang)
      ...
      '<div class="form-group {{ $errors->has('description_en') ? ' has-error' : '' }}">' +
        ...
        '<textarea name="accordions_{{ $lang }}[' + (count + 1) + '][description]"
          id="description_{{ $lang }}_' + (count + 1) + '" cols="30" rows="3" class="form-control"></textarea>'
        +
        '</div>' +
      '</div>' +
      '
      ...
      '<div class="dynamic_field">' +
        ...
        '<div class="form-group">' +
          ...
          '<textarea name="employees_{{ $lang }}[' + (count + 1) + '][description]"' +
                                                                    '
            id="employees_{{ $lang }}_' + (count + 1) + '" cols="30"' +
                                                                    ' rows="3"' +
                                                                    ' class="form-control"></textarea>' +
          '</div>' +
        '</div>' +
