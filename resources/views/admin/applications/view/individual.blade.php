                        ...
                        <div class="x_panel">
                            ...
                            <div class="x_content">
                                <div class="form-group">
                                    <label>Тип</label>
                                    <input type="text" class="form-control" value="Участник" disabled>
                                </div>
                                <div class="form-group">
                                    <label>Регион</label>
                                    <input type="text" class="form-control" value="{{ $item->region ? $item->region : "Не указан" }}" disabled>
                                </div>
                                <div class="form-group">
                                ...
                                <div class="form-group">
                                    <label>Документ, подтверждающий пенсионные отчисления сотрудника (не менее трех месяцев). Информация конфиденциальна, и не предоставляется третьим лицам</label>
                                    <br><a href="{{ $item->document_1 }}" target="_blank" class="btn btn-primary">{{ $item->document_1 }}</a>
                                </div>
                                <div class="form-group">
                                    <label>Заполненная техническая заявка, установленной формы по виду спорта (Приложение №1 к настоящему Регламенту)</label>
                                    <br><a href="{{ $item->document_2 }}" target="_blank" class="btn btn-primary">{{ $item->document_2 }}</a>
                                </div>
                                <div class="form-group">
                                    <label>Документ, подтверждающий личность (удостоверение личности или паспорт – на каждого участника Спартакиады)</label>
                                    <br><a href="{{ $item->document_3 }}" target="_blank" class="btn btn-primary">{{ $item->document_3 }}</a>
                                </div>
                                <div class="form-group">
                                    <label>Заявление о личной ответственности на каждого участника Спартакиады (Приложение №3 к настоящему Регламенту)</label>
                                    <br><a href="{{ $item->document_4 }}" target="_blank" class="btn btn-primary">{{ $item->document_4 }}</a>
                                </div>
                                <div class="form-group">
                                    <label>Статус</label>
