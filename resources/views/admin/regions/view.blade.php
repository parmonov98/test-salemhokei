    ...
    <div class="right_col" role="main">
        <div class="">
            ...
            <div class="row">
                ...
                @endif
                <form action="/admin/regions/{{ $item->id ? $item->id : "add" }}" method="post"
                                            ...
                                            <div role="tabpanel" class="tab-pane fade @if($key == 0) active in @endif"
                                                ...
                                                <div class="form-group {{ $errors->has("name_ru") ? " has-error" : "" }}">
                                                    ...
                                                    <input type="text" name="name_{{$language}}"
                                                           id="{{ $item->getAttribute("name_$language") }}"
                                                           class="form-control"
                                                           placeholder="Введите название города"
                                                           value="{{ $item->getAttribute("name_$language") }}">
                                                    @if ($item->getAttribute("name_$language"))
                ...
                @if(!empty($item->id))
                    <form action="{{ route('regions.delete', $item->id) }}" method="POST">
                        {{ csrf_field() }}
                        {{ method_field('DELETE') }}
                        <div class="col-md-12 col-sm-12 col-xs-12">
